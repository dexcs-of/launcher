...
class dexcsCfdPreferencePage:
    ...
    def setDownloadURLs(self):
        if self.testGetRuntime() == "MinGW":
            foam_ver = os.path.split(dexcsCfdTools.getFoamDir())[-1]
    ...
    def chooseTemplateCase(self):
        d = QtGui.QFileDialog().getExistingDirectory(None, 'Choose template case', self.template_case)
        if d and os.access(d, os.W_OK):
    ...
    def runDependencyChecker(self):
        ...
        msg = dexcsCfdTools.checkCfdDependencies()
        if not msg:
            self.consoleMessage("No missing dependencies detected")
        else:
            self.consoleMessage(msg)
        dexcsCfdTools.setFoamDir(self.initial_foam_dir)
        dexcsCfdTools.setParaviewPath(self.initial_paraview_path)
        QApplication.restoreOverrideCursor()
    ...
    def createThread(self):
        if self.thread and self.thread.isRunning():
            ...
            self.thread = dexcsCfdPreferencePageThread()
    ...
    def threadFinished(self, status):
        if self.thread.task == DOWNLOAD_CFMESH:
            if status:
                if dexcsCfdTools.getFoamRuntime() != "MinGW":
                    ...
                    if dexcsCfdTools.getFoamRuntime() == 'WindowsDocker':
                        ...
                        self.install_process = dexcsCfdTools.startFoamApplication(
...
class dexcsCfdPreferencePageSignals(QObject):
...
class dexcsCfdPreferencePageThread(QThread):
    """ Worker thread to complete tasks in preference page """
    def __init__(self):
        super(dexcsCfdPreferencePageThread, self).__init__()
        self.signals = dexcsCfdPreferencePageSignals()
        self.user_dir = None
        self.task = None
    ...
    def downloadCfMesh(self):
        ...
        if dexcsCfdTools.getFoamRuntime() == "MinGW":
            ...
            dexcsCfdTools.runFoamCommand(
                '{{ mkdir -p "$FOAM_APPBIN" && cd "$FOAM_APPBIN" && unzip -o "{}"; }}'.
                    format(dexcsCfdTools.translatePath(filename)))
        else:
            self.user_dir = dexcsCfdTools.runFoamCommand("echo $WM_PROJECT_USER_DIR").rstrip().split('\n')[-1]
            # We can't reverse-translate the path for docker since it sits inside the container. Just report it as such.
            if dexcsCfdTools.getFoamRuntime() != 'WindowsDocker':
                self.user_dir = dexcsCfdTools.reverseTranslatePath(self.user_dir)
                    ...
                    with tempfile.TemporaryDirectory() as tempdir:
                        ...
                        dexcsCfdTools.runFoamCommand(
                            '{{ mkdir -p "$WM_PROJECT_USER_DIR" && cp -r "{}" "$WM_PROJECT_USER_DIR/"; }}'
                                .format(dexcsCfdTools.translatePath(os.path.join(tempdir, CFMESH_FILE_BASE))))
            else:
                dexcsCfdTools.runFoamCommand(
                    '{{ mkdir -p "$WM_PROJECT_USER_DIR" && cd "$WM_PROJECT_USER_DIR" && ( rm -r {}; unzip -o "{}"; ); }}'.
                    format(CFMESH_FILE_BASE, dexcsCfdTools.translatePath(filename)))
            ...
            if dexcsCfdTools.getFoamRuntime() == 'WindowsDocker':
                ...
                with ZipFile(filename, 'r') as zip:
                    with tempfile.TemporaryDirectory() as tempdir:
                        ...
                        dexcsCfdTools.runFoamCommand(
                            '{{ mkdir -p "$WM_PROJECT_USER_DIR" && cp -r "{}" "$WM_PROJECT_USER_DIR/"; }}'
                                .format(dexcsCfdTools.translatePath(os.path.join(tempdir, HISA_FILE_BASE))))
            else:
                dexcsCfdTools.runFoamCommand(
                    '{{ mkdir -p "$WM_PROJECT_USER_DIR" && cd "$WM_PROJECT_USER_DIR" && ( rm -r {}; unzip -o "{}"; );  }}'.
                    format(HISA_FILE_BASE, dexcsCfdTools.translatePath(filename)))
