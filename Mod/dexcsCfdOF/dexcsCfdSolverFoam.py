...
class _CommandCfdSolverFoam:
    def GetResources(self):
        icon_path = os.path.join(dexcsCfdTools.get_module_path(), "Gui", "Resources", "icons", "solver.png")
        return {'Pixmap': icon_path,
                'MenuText': QtCore.QT_TRANSLATE_NOOP("Cfd_SolverControl", "Solver job control"),
                'Accel': "S, C",
                'ToolTip': QtCore.QT_TRANSLATE_NOOP("Cfd_SolverControl", _("Edit properties and run solver"))}
    ...
    def Activated(self):
        ...
        if not isPresent:
            ...
            FreeCADGui.doCommand("dexcsCfdTools.getActiveAnalysis().addObject(CfdSolverFoam.makeCfdSolverFoam())")
...
class _CfdSolverFoam(object):
    ...
    def __init__(self, obj):
        ...
        addObjectProperty(obj, "InputCaseName", "", "App::PropertyFile", "Solver",
...
class _ViewProviderCfdSolverFoam:
    ...
    def setEdit(self, vobj, mode):
        if dexcsCfdTools.getActiveAnalysis():
            from dexcsCfdRunnableFoam import CfdRunnableFoam
            foamRunnable = CfdRunnableFoam(dexcsCfdTools.getActiveAnalysis(), self.Object)
            from _dexcsTaskPanelCfdSolverControl import _dexcsTaskPanelCfdSolverControl
            taskd = _dexcsTaskPanelCfdSolverControl(foamRunnable)
