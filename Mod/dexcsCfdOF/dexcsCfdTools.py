...
def getSolver(analysis_object):
    from dexcsCfdSolverFoam import _CfdSolverFoam
    for i in analysis_object.Group:
...
def getPreferencesLocation():
    ...
    return "User parameter:BaseApp/Preferences/Mod/dexcsCfdOF"
...
def makeRunCommand(cmd, dir, source_env=True):
    ...
    #print("debug_makeRunCommand",dir)
    ...
    print('getFoamRunTime',getFoamRuntime())
...
class CfdSynchronousFoamProcess:
    def __init__(self):
        self.process = dexcsCfdConsoleProcess.CfdConsoleProcess(stdoutHook=self.readOutput, stderrHook=self.readOutput)
        self.output = ""
...
def startFoamApplication(cmd, case, log_name='', finishedHook=None, stdoutHook=None, stderrHook=None):
    ...
    proc = dexcsCfdConsoleProcess.CfdConsoleProcess(finishedHook=finishedHook, stdoutHook=stdoutHook, stderrHook=stderrHook)
    if logFile:
