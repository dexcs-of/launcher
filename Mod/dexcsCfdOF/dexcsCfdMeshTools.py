...
class CfdMeshTools:
    def __init__(self, cart_mesh_obj):
        ...
        output_path = dexcsCfdTools.getOutputPath(self.analysis)
    ...
    def processDimension(self):
        ...
        twoDPlanes = []
        analysis_obj = dexcsCfdTools.getParentAnalysisObject(self.mesh_obj)
        if not analysis_obj:
            analysis_obj = dexcsCfdTools.getActiveAnalysis()
        if analysis_obj:
            boundaries = dexcsCfdTools.getCfdBoundaryGroup(analysis_obj)
            for b in boundaries:
        ...
        if self.dimension == '2D':
            ...
            fbi = dexcsCfdTools.getCfdBoundaryGroup(analysis_obj).index(frontObj)
            ffl = []
            ...
            bbi = dexcsCfdTools.getCfdBoundaryGroup(analysis_obj).index(backObj)
            bfl = []
    ...
    def processRefinements(self):
        ...
        mr_objs = dexcsCfdTools.getMeshRefinementObjs(self.mesh_obj)
        ...
        dexcsCfdTools.cfdMessage("Matching boundary patches\n")
        bc_group = None
        analysis_obj = dexcsCfdTools.getParentAnalysisObject(self.mesh_obj)
        if not analysis_obj:
            analysis_obj = dexcsCfdTools.getActiveAnalysis()
        if analysis_obj:
            bc_group = dexcsCfdTools.getCfdBoundaryGroup(analysis_obj)
        boundary_face_list = []
        for bc_id, bc_obj in enumerate(bc_group):
            for ri, ref in enumerate(bc_obj.References):
                try:
                    bf = dexcsCfdTools.resolveReference(ref)
        ...
        bc_matched_faces = dexcsCfdTools.matchFaces(boundary_face_list, mesh_face_list)

        # Make list of all boundary layer mesh regions for cfMesh
        bl_matched_faces = []
        if self.mesh_obj.MeshUtility == 'cfMesh':
            dexcsCfdTools.cfdMessage("Matching boundary layer regions\n")
            ...
            for mr_id, mr_obj in enumerate(mr_objs):
                if mr_obj.NumberLayers > 1 and not mr_obj.Internal:
                    for ri, r in enumerate(mr_obj.References):
                        try:
                            f = dexcsCfdTools.resolveReference(r)
                        except RuntimeError as re:
            ...
            # Match them up
            bl_matched_faces = dexcsCfdTools.matchFaces(bl_face_list, mesh_face_list)
        ...
        if self.mesh_obj.MeshUtility == 'snappyHexMesh':
            dexcsCfdTools.cfdMessage("Matching surface geometries\n")
            ...
            bc_mr_matched_faces = dexcsCfdTools.matchFaces(boundary_face_list, mr_face_list)
        ...
        for bc_id, bc_obj in enumerate(bc_group):
            if bc_obj.BoundaryType == 'baffle':
                ...
                for ri, refs in enumerate(baffle_patch_refs):
                    try:
                        shape = dexcsCfdTools.makeShapeFromReferences(refs)
                    except RuntimeError as re:
                        raise RuntimeError("Error processing baffle {}: {}".format(
                            bc_obj.Label, str(re)))
                    solid_name = bc_obj.Name + "_" + str(ri)
                    if shape:
                        dexcsCfdTools.cfdMessage("Triangulating baffle {}, section {} ...".format(bc_obj.Label, ri))
                        facemesh = MeshPart.meshFromShape(
                            shape, LinearDeflection=self.mesh_obj.STLLinearDeflection)

                        dexcsCfdTools.cfdMessage(" writing to file\n")
                        with open(os.path.join(self.triSurfaceDir, solid_name + '.stl'), 'w') as fid:
                            dexcsCfdTools.writePatchToStl(solid_name, facemesh, fid, self.scale)

                        if ri > 0:  # The parts of the baffle corresponding to a surface mesh region obj
                            mr_obj = mr_objs[ri-1]
                            refinement_level = dexcsCfdTools.relLenToRefinementLevel(mr_obj.RelativeLength)
                            edge_level = dexcsCfdTools.relLenToRefinementLevel(mr_obj.RegionEdgeRefinement)
        ...
        if self.mesh_obj.MeshUtility == 'snappyHexMesh':
            ...
            mr_matched_faces = dexcsCfdTools.matchFaces(mr_face_list, mesh_face_list)
        ...
        for mr_id, mr_obj in enumerate(mr_objs):
            ...
            for bi in range(len(mr_patch_refs)):
                if len(mr_patch_refs[bi]):
                    ...
                    dexcsCfdTools.cfdMessage("Triangulating mesh refinement region {}, section {} ...".format(
                        mr_obj.Label, bi))

                    try:
                        shape = dexcsCfdTools.makeShapeFromReferences(mr_patch_refs[bi])
                    except RuntimeError as re:
                    ...
                    if shape:
                        ...
                        with open(os.path.join(self.triSurfaceDir, mr_patch_name + '.stl'), 'w') as fid:
                            dexcsCfdTools.writePatchToStl(mr_patch_name, facemesh, fid, self.scale)
                    ...
                    if self.mesh_obj.MeshUtility == 'cfMesh':
                        if not Internal:
                            ...
                            #    'RelativeLength': mr_rellen * self.clmax * self.scale
                                'RelativeLength': self.scale * Units.Quantity(mr_rellen.RelativeLength).Value
                            }
                        ...
                        if mr_obj.References:
                            for sub in mr_obj.References:
                                ...
                                if search_ele_in_shape_to_mesh:
                                    ...
                                    found_element = dexcsCfdTools.findElementInShape(self.part_obj.Shape, ele_shape)
                                    if found_element:
                                        elems = found_element
                                    else:
                ...
                for k in range(len(self.patch_faces)):
                    for l in range(len(self.patch_faces[k])):
                        ...
                        if len(patch_faces):
                            ...
                            dexcsCfdTools.cfdMessage(" writing to file\n")
                            dexcsCfdTools.writePatchToStl(patch_name, mesh_stl, fid, self.scale)
    ...
    def writeMeshCase(self):
        ...
        dexcsCfdTools.cfdMessage("Populating mesh dictionaries in folder {}\n".format(self.meshCaseDir))
        ...
        if self.mesh_obj.MeshUtility == "cfMesh":
            ...
            snappy_settings['EdgeRefinementLevel'] = dexcsCfdTools.relLenToRefinementLevel(self.mesh_obj.EdgeRefinement)
            ...
            self.gmsh_settings['Executable'] = dexcsCfdTools.translatePath(exe)
        ...
        self.template_path = os.path.join(dexcsCfdTools.get_module_path(), "data", "defaultsMesh")
        ...
        self.settings = {
            ...
            'FoamRuntime': dexcsCfdTools.getFoamRuntime(),
        ...
        if dexcsCfdTools.getFoamRuntime() != 'WindowsDocker':
            self.settings['TranslatedFoamPath'] = dexcsCfdTools.translatePath(dexcsCfdTools.getFoamDir())
        ...
        dexcsCfdTools.cfdMessage("Successfully wrote meshCase to folder {}\n".format(self.meshCaseDir))
